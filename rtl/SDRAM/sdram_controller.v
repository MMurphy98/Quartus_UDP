...
module sdram_controller(
    ...
    input  [24:0] sdram_wr_addr,	//SDRAM写操作的地址
    input  [10:0] sdram_wr_burst,   //写sdram时数据突发长度
    input  [31:0] sdram_din,	    //写入SDRAM的数据
    
	//SDRAM 控制器读端口	
    input         sdram_rd_req,		//读SDRAM请求信号
    output        sdram_rd_ack,		//读SDRAM响应信号
    input  [24:0] sdram_rd_addr,	//SDRAM写操作的地址
    input  [10:0] sdram_rd_burst,   //读sdram时数据突发长度
...
// SDRAM 数据读写模块
sdram_data u_sdram_data_low16bit(		
    .clk                (clk),
    .rst_n              (rst_n),
    
    .sdram_data_in      (sdram_din[15:0]),
    .sdram_data_out     (sdram_dout[15:0]),
    .work_state         (work_state),
    .cnt_clk            (cnt_clk),
    
    .sdram_data         (sdram_data[15:0])
    );

sdram_data u_sdram_data_high16bit(		
    .clk                (clk),
    .rst_n              (rst_n),
    
    .sdram_data_in      (sdram_din[31:16]),
    .sdram_data_out     (sdram_dout[31:16]),
    .work_state         (work_state),
    .cnt_clk            (cnt_clk),
    
    .sdram_data         (sdram_data[31:16])
    );
