...
module sdram_ctrl(
    ...
    input      [10:0] sdram_wr_burst,	//突发写SDRAM字节数（1-512个）
    input      [10:0] sdram_rd_burst,	//突发读SDRAM字节数（1-256个）	
...
parameter  TRSC_CLK	  = 10'd6;	        //模式寄存器设置时钟周期，至少14ns (TMRD)
parameter  TRCD_CLK	  = 10'd2;	        //行选通周期，至少15ns，
parameter  TCL_CLK	  = 10'd3;	        //列潜伏期(cl)，3个TCK周期 (TCAC)
parameter  TWR_CLK	  = 10'd2;	        //写入校正，TWR+TRCD=TARS > 37 ns
