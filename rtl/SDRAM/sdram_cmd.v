...
module sdram_cmd(
    ...
    input      [10:0] sdram_wr_burst,	//突发写SDRAM字节数
    input      [10:0] sdram_rd_burst,	//突发读SDRAM字节数
    
    input      [ 4:0] init_state,		//SDRAM初始化状态
    input      [ 3:0] work_state, 		//SDRAM工作状态
    input      [10:0] cnt_clk,		    //延时计数器	
            ...
            `I_DONE:                    //SDRAM初始化完成
					case(work_state)    //以下工作状态不执行任何指令
                        ...
                        `W_ACTIVE: begin//行有效指令
                                ...
                                sdram_ba    <= sys_addr[24:23];
                                sdram_addr  <= sys_addr[22:10];
                            end
                        `W_READ: begin  //读操作指令
                                sdram_cmd_r <= `CMD_READ;
                                sdram_ba    <= sys_addr[24:23];
                                sdram_addr  <= {3'b000,sys_addr[9:0]};
                            end
                        `W_RD: begin    //突发传输终止指令
                                if(`end_rdburst) 
